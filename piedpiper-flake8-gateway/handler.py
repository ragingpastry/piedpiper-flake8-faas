from .util import request_new_task_id, call_flake8_function, update_task_id_status
...
def handle(request):
    ...
    return_value = call_flake8_function(run_id=run_id,
                                        faas_url="http://172.17.0.1:8080/async-function/piedpiper-flake8-function",
                                        project=project,
    ...
    if return_value.status_code == 202:
        update_task_id_status(gman_url=gman_url, task_id=internal_task_id,
                              status='delegated', message='Delegated execution to flake8 executor')
